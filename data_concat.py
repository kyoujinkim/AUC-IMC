...
consenlist = glob('./data/korea/consenlist*.csv')
for idx, file in enumerate(consenlist):
    if idx == 0:
        df = pd.read_csv(file)
    else:
        df = pd.concat([df, pd.read_csv(file)], ignore_index=True, axis=0)
df = df.dropna(subset=['BPS', 'E_EPS', 'A_EPS'])
df = df.drop_duplicates()

gdp = pd.read_excel('./data/korea/QGDP.xlsx', sheet_name='Sheet1', header=13, index_col=0, parse_dates=True).dropna(how='all', axis=0).dropna(axis=1)
...
df.to_csv('./data/korea/total.csv', index=False, encoding='utf-8-sig')
